...
import { Settings, Tile } from './interfaces';
...
const createWindow = (): void => {
    ...
    ipcMain.handle('getAvailableCommands', async () => await agentService.getAvailableCommands());
    ipcMain.handle('getPluginStatus', () => pluginsStatus);
    ipcMain.handle('togglePlugin', async (_, ns: string) => await agentService.togglePlugin(ns));
    ipcMain.handle('getAgentData', async () => await firstValueFrom(agentService.dataObservable));
    ipcMain.handle('updateSettings', async (_, settings: Settings) => await agentService.updateSettings(settings));
    ipcMain.handle('upsertTile', async (_, tile: Tile) => await agentService.upsertTile(tile));
    ipcMain.handle('removeTile', async (_, id: string) => await agentService.removeTile(id));
    ipcMain.handle('updateSortOrder', async (_, order: string[]) => await agentService.updateSortOrder(order));
};
